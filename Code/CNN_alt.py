...
def load_dataset():
    ...
    testY = to_categorical(testY)
    return trainX, trainY, val_X, val_Y, testX, testY
...
def prep_pixels_original(train, test):
    # convert from integers to floats
    train_norm = train.astype('float32')
    test_norm = test.astype('float32')
    # normalize to range 0-1
...
def run_test_harness_original():
    # load dataset
    trainX, trainY, testX, testY = load_dataset_original()

    # prepare pixel data
    trainX, testX = prep_pixels_original(trainX, testX)
    # define model
    model = define_model()
    # # fit model
...
# run_test_harness_original()
start = time.time()
run_test_harness()
finish = time.time()
print("Training time: " + str(finish - start) + " seconds")
